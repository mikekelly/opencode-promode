 ...
 */
export function createContextMonitorHook(_pluginCtx: PluginInput) {
  ...
  return {
    ...
    "tool.execute.after": async (
      ...
      if (Date.now() - state.lastUpdateTime > 60000) return
...
`)
     ...
     */
    updateUsage: (usage: number, limit?: number) => {
      state.lastKnownUsage = usage
      if (limit) state.lastKnownLimit = limit
      state.lastUpdateTime = Date.now()
    },
  }
}
