...
<critical-instruction>
You are a sub-agent. You MUST NOT delegate tasks or spawn other agents. Execute tasks yourself and report back to the main agent.
</critical-instruction>
...
5. **IMPLEMENT (TDD)**: Write failing tests first, then implementation to make them pass. Never write implementation code without a failing test.
 ...
 */
export function createPromodeSubagent(model: string = DEFAULT_MODEL): AgentConfig {
  return {
    description:
      "General-purpose subagent that follows promode conventions. Use for ANY task delegation - this subagent understands TDD, progressive disclosure, context conservation, and all promode development principles.",
