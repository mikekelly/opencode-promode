 ...
 */
export function createSelfCompactTool(pluginCtx) {
    return tool({
...
Only use this when context usage is high (>70%) and you have externalized important state.`,
        args: {
            confirm_externalized: tool.schema
                .boolean()
                .describe("Confirm that you have saved important context to files before compacting. Set to true only after externalizing state."),
            externalized_files: tool.schema
                .array(tool.schema.string())
                .optional()
                .describe("List of files where you saved important context (e.g., ['.context/task-state.md', 'TODO.md'])"),
            summary_hint: tool.schema
                .string()
                .optional()
                .describe("Optional hint for the compaction summary - what should be preserved in the summary"),
        },
        async execute(args, toolCtx) {
            ...
            if (!args.confirm_externalized) {
                return JSON.stringify({
                    success: false,
                    error: "You must externalize important context to files before compacting. " +
            ...
            try {
                ...
                return JSON.stringify({
                    ...
                    suggestion: "Check if the session is active and try again. If the problem persists, " +
...
//# sourceMappingURL=self-compact.js.map